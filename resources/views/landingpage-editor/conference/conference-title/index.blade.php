            ...
            <tbody>
                @foreach ($conferences as $index => $conference)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('landing.conference.edit', $conference->id) }}" class="btn btn-warning btn-sm">Edit</a>
                            <form action="{{ route('landing.conference.destroy', $conference->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this?')">
                                @csrf
                                @method('DELETE')
