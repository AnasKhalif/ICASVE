...
@section('content')
    <div class="container card p-4">
        ...
        <form action="{{ route('landing.abouts.update', $about->id) }}" method="POST" enctype="multipart/form-data" id="editAboutForm">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label class="form-label">Title</label>
                <input type="text" name="title" class="form-control" value="{{ $about->title }}" required placeholder="Example: About Our Company">
            </div>
            <div class="mb-3">
                <label class="form-label">Content</label>
                <textarea name="content" class="form-control" rows="5" required placeholder="Example: Our company was founded in 2020 with the vision to provide the best solutions...">{{ $about->content }}</textarea>
            </div>
            <div class="mb-3">
                <label class="form-label">Year</label>
                <input type="number" name="year" class="form-control" value="{{ $about->year }}" required min="2024" max="{{ date('Y') }}" placeholder="Example: 2024">
