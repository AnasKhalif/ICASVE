        ...
        <tbody>
            @foreach($committees as $index => $committee)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('landing.organizing.edit', $committee->id) }}" class="btn btn-warning btn-sm">Edit</a>
                        <form action="{{ route('landing.organizing.destroy', $committee->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure?');">
                            @csrf
                            @method('DELETE')
