        ...
        <tbody>
            @foreach($committees as $index => $committee)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('landing.steering.edit', $committee->id) }}" class="btn btn-warning btn-sm">Edit</a>

                        <form action="{{ route('landing.steering.destroy', $committee->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                            @csrf
                            @method('DELETE')
